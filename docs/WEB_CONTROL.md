...
* **Видео в браузере** — есть **снимок** локального источника
  (`/api/frame.png`, `/api/video.mjpeg`), но **нет** WebRTC/SFU: удалённые
  участники в браузере не показываются, звука нет. Полноценный WebRTC —
  отдельный крупный этап (см. ADR-0001, §5).
