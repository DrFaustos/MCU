...
| Видео в звонке | 🚧 | включено; требуются два терминала для проверки. На Wayland встраивание в тайл может не работать — открывается отдельным окном |
...
* 🚧 H.323 — `h323_gateway.py` является заглушкой/обёрткой; для реальной
  работы нужны GStreamer/openh323 и отдельная настройка.
* ❌ NAT traversal (STUN/TURN/ICE) для работы через интернет.
* 🚧 Шифрование SRTP/TLS — опционально и по умолчанию выключено.
* ❌ Адаптивный битрейт по RTCP (packet loss/jitter).
