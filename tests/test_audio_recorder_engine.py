...
def test_engine_stop_calls_audio_recorder_stop():
    ...
    engine._audio_recorder._is_recording = True
    engine._audio_recorder._recorder = object()
    engine._audio_recorder._call_media = None  # stopTransmit пропустится

    called = {"n": 0}
    orig_stop = engine._audio_recorder.stop_recording

    def spy():
        called["n"] += 1
        return orig_stop()

    engine._audio_recorder.stop_recording = spy
    engine._running = True  # чтобы stop() не вышел сразу
    engine.stop()
    assert called["n"] == 1
    assert engine._audio_recorder.is_recording is False
