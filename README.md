...
* **встроенная web-панель управления** (`--web`): браузер подключается к
  приложению и управляет сессией — участники, вызовы, муты, раскладка,
  запись, чат, устройства ([docs/WEB_CONTROL.md](docs/WEB_CONTROL.md));
* **web-конференция из браузера** (как BBB): любой участник открывает
  страницу, вводит **имя** и присоединяется; браузер сам подхватывает
  камеру/микрофон (`getUserMedia`) и публикует их в MCU, а сервер раздаёт
  видео и **аудио** другим веб-участникам (WebRTC, опционально `aiortc`);
* **TLS (HTTPS)** для web-панели — опционально, по умолчанию выключено
  (`--web-tls`, самоподписанный сертификат);
* **ICE (STUN/TURN)** для WebRTC — настраивается для работы через интернет;
...
Позвонить из клиента: выбрать протокол в списке **Протокол**, ввести
SIP/H.323 URI (например `sip:100@192.168.1.50`) или IP-адрес и нажать
**Call**.

### Выбор протокола при звонке

Исходящий вызов можно направить по одному из протоколов — это нужно для
тестов совместимости (Sony/Polycom) и для отладки:

| Протокол | Что использует | Когда выбирать |
|----------|----------------|----------------|
| **Авто** | `sip:`/`h323:` в адресе | по умолчанию; голый IP → SIP |
| **SIP (PJSIP)** | `pjsua2` | обычные SIP-терминалы |
| **H.323 — шлюз (GStreamer)** | `h323_gateway.py` | legacy-терминалы через внешний шлюз |
| **H.323 — нативный (mcu_h323d)** | H323Plus через C++-хост | аппаратные ВКС (Sony/Polycom) без шлюза |

В GUI протокол выбирается в выпадающем списке **Протокол** рядом с полем
ввода адреса. В режиме **Авто** адрес с префиксом `h323:` идёт по H.323
(нативный стек, если хост `mcu_h323d` запущен, иначе — GStreamer-шлюз),
а `sip:` и голый IP — по SIP.

То же из командной строки:

    # исходящий H.323 по нативному стеку (mcu_h323d) на IP аппаратного терминала
    ...
    # GUI с предустановленным протоколом
    python run.py --proto h323_native --auto-call 192.168.1.60

Флаг `--proto` (или `features.default_call_protocol` в `config.json`)
задаёт протокол по умолчанию и для GUI, и для `--call`/`--auto-call`.
Подробнее — в [docs/CALL_PROTOCOL.md](docs/CALL_PROTOCOL.md).
...
* `features.default_call_protocol` — протокол исходящего вызова по умолчанию
  (`auto` / `sip` / `h323` / `h323_native`); `auto` определяет по префиксу адреса;
* `features.layouts.available` — список доступных раскладок видео;
* `features.virtual_camera` — единый источник видео (коммутатор + v4l2loopback/OBS),
  по умолчанию **false**; `features.virtual_camera_device` — путь к устройству;
* `features.web` — встроенный web-сервер: `enabled`, `host`, `port`, `auth_token`,
  `tls`, `cert_file`, `key_file`, `ice_servers`, `turn_user`, `turn_password`
  (см. [docs/WEB_CONTROL.md](docs/WEB_CONTROL.md));
...
## Структура проекта
    ...
    │   ├── web_server.py          # встроенный web-сервер (REST/SSE)
    │   ├── webui/index.html       # страница web-панели и конференции
    │   ├── webrtc_ingest.py       # WebRTC: приём/раздача медиа (aiortc, опц.)
    │   ├── webrtc_sfu.py          # конференц-ядро: участники + шина медиа
    │   ├── video_stream.py        # кадры источника -> PNG/MJPEG (FrameHub)
    │   ├── tls_utils.py           # самоподписанный TLS-сертификат (web)
...
* кнопка **Тест камеры** — открывает локальное превью камеры;
* выпадающий список **Микрофон** — выбор устройства захвата;
* кнопка **Открыть монитор микрофона** — окно с живым эквалайзером
  (видно уровень сигнала);
* чекбоксы **Камера** и **Микрофон** — включают/выключают устройства
  до и во время звонка.

В сетке видео всегда есть **тайл «Вы»** (своя камера):

* **клик по тайлу** — включить/выключить локальное превью камеры;
* **селектор камеры прямо в тайле** — сменить источник, синхронизирован
  с правой панелью;
* **мут видео на своём тайле** — только передача: удалённые видят «нет
  видео», локально камера остаётся видна;
* есть **единый источник** (`features.virtual_camera`): камера читается
  один раз и отдаётся через виртуальную камеру (v4l2loopback/OBS).
...
| Монитор микрофона (уровень) | **Открыть монитор микрофона**     |
...
ошибки и необработанные исключения с трассировкой, а также трассировку
фатальных сигналов (`faulthandler` — segfault/abort в pjsua2/Qt).

Это особенно важно на **Windows** в сборке `--windowed`: консоли нет, и если
приложение закрывается сразу после запуска, причина останется в
`mcu-client.log` рядом с `.exe`. Дополнительно при **критической ошибке на
старте** (импорт, конфиг, движок) показывается нативное окно **MessageBox**
с текстом и путём к лог-файлу — приложение не «исчезает молча»
(`mcuclient.log.report_fatal`).
...
| Протокол вызова             | список **Протокол** / `--proto` |
...
Через **web-панель** (`--web`) доступны те же действия из браузера, а также
**вход в конференцию по имени** и публикация своей камеры/микрофона
(WebRTC) — см. [docs/WEB_CONFERENCE_TEST.md](docs/WEB_CONFERENCE_TEST.md).
...
* Web-конференция (браузер) — это **приём + fan-out** медиа, а не полноценный
  SFU: нет записи веб-потока, симулкаста и микширования аудио (каждый зритель
  получает отдельный трек на каждого публикатора); для интернета/NAT нужен
  STUN/TURN. Раздача SIP/H.323-видео в браузер напрямую — нет (нужен транскодинг).
...
debug-бинарники** (консольные, `MCU_DEBUG=1`, расширенный лог):
`MCU-Client-debug.exe` (Windows) и `MCU-Client-debug` (Linux) — и
прикрепляет их к GitHub Release. Это сделано для этапа активной отладки:
в консольной сборке видны логи сразу в окне. Флаг `PYTHONUTF8=1` в workflow
устраняет ошибку `UnicodeEncodeError` на Windows-раннере (cp1252).

Локально: `python build.py --debug-only` — собрать только debug-бинарник.
...
Проверка web-конференции из браузера — [docs/WEB_CONFERENCE_TEST.md](docs/WEB_CONFERENCE_TEST.md).
