...
Видео включается в PJSIP явно (PJMEDIA_HAS_VIDEO=1) — скрипт установки
делает это автоматически. Проверить доступные камеры и протестировать их
можно ДО приёма звонка:

    # список видеоустройств (камера, SDL, colorbar)
    python run.py --list-video-devices

    # локальное превью камеры id=0 на 5 секунд
    python run.py --test-camera 0 --preview-seconds 5

    # выбрать камеру для звонка и стартовать с выключенным микрофоном
...
| Действие                    | CLI / UI                          |
|-----------------------------|-----------------------------------|
| Список камер                | `--list-video-devices`            |
| Тест камеры до звонка       | `--test-camera ID`                |
| Выбор камеры                | `--camera-device ID`              |
| Старт без камеры            | `--no-camera`                     |
| Старт без микрофона         | `--no-mic`                        |
| Вкл/выкл камеру в UI        | тумблер **Камера**                |
| Вкл/выкл микрофон в UI      | тумблер **Микрофон**              |
