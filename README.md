...
## Локальный тестовый стенд (без второй машины)

Чтобы не переключаться между ОС и не звонить самому себе вручную, есть
скрипт, который поднимает **два headless-инстанса MCU на 127.0.0.1** и
проверяет старт/остановку движка и события:

 

Скрипт работает в stub-режиме (даже если `pjsua2` не установлен) и
возвращает ненулевой код при сбое — годится для CI.

Переменные окружения: `PYTHON`, `PORT_A`, `PORT_B`, `LOG_DIR`.

> **Контейнеры (podman/docker/distrobox) в этом окружении недоступны:**
> воркспейс сам является непривилегированным контейнером (нулевые
> capabilities, `unshare --net` запрещён ядром), поэтому вложенный
> контейнерный рантайм и виртуальные машины (GNOME Boxes/Windows 11)
> запустить нельзя. Стенд на процессах — рабочий путь здесь.
